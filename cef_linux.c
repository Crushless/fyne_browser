...
struct fynecef_browser_s {
  ...
  cef_window_handle_t host_window;
  int x;
  int y;
  int width;
  int height;
...
static void fynecef_destroy_host_window(struct fynecef_browser_s* owner) {
  if (owner == NULL || owner->host_window == 0) {
    return;
  }

  XDisplay* display = cef_get_xdisplay();
  if (display != NULL) {
    XDestroyWindow(display, (Window)owner->host_window);
    XFlush(display);
  }
  owner->host_window = 0;
}

static void fynecef_embed_browser_window(struct fynecef_browser_s* owner,
                                         int width,
                                         int height) {
  if (owner == NULL || owner->browser == NULL || owner->host_window == 0) {
    return;
  }

  cef_browser_host_t* host = owner->browser->get_host(owner->browser);
  XDisplay* display = cef_get_xdisplay();
  cef_window_handle_t browser_window = 0;

  if (host == NULL || display == NULL) {
    return;
  }

  if (host->get_window_handle != NULL) {
    browser_window = host->get_window_handle(host);
  }
  if (browser_window == 0 || browser_window == owner->host_window) {
    return;
  }

  XReparentWindow(display, (Window)browser_window, (Window)owner->host_window,
                  0, 0);
  XMoveResizeWindow(display, (Window)browser_window, 0, 0,
                    (unsigned int)(width > 0 ? width : 1),
                    (unsigned int)(height > 0 ? height : 1));
  XMapWindow(display, (Window)browser_window);
  XFlush(display);
}
...
static int fynecef_owner_release(struct fynecef_browser_s* owner) {
  ...
  if (refs == 0) {
    ...
    fynecef_destroy_host_window(owner);
    free(owner);
    return 1;
  }
  return 0;
...
static void CEF_CALLBACK fynecef_on_before_close(cef_life_span_handler_t* self,
  ...
  fynecef_destroy_host_window(wrapper->owner);
...
int fynecef_initialize(int argc,
  ...
  fynecef_string_from_utf8(locales_dir, &settings.locales_dir_path);
  ...
  fynecef_clear_string(&settings.locales_dir_path);
...
fynecef_browser_t* fynecef_browser_create(uintptr_t go_handle,
  ...
  owner->parent_window = (cef_window_handle_t)parent_window;
  owner->x = x;
  owner->y = y;
  ...
  window_info.parent_window = (cef_window_handle_t)parent_window;

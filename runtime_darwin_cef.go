...
func nativeParentHandle(window fyne.Window) (uintptr, error) {
	...
	nativeWindow.RunNative(func(ctx any) {
		...
		case driver.MacWindowContext:
			handle = uintptr(C.fynecef_window_content_view(C.uintptr_t(value.NSWindow)))
		default:
...
func prepareLinuxRuntimeLayout(frameworkRoot string) (linuxRuntimeLayout, error) {
	...
	frameworkDir := filepath.Join(releaseDir, "Chromium Embedded Framework.framework")
	resourcesDir := filepath.Join(frameworkDir, "Resources")

	requiredPaths := []string{
		filepath.Join(frameworkDir, "Chromium Embedded Framework"),
		filepath.Join(resourcesDir, "chrome_100_percent.pak"),
		filepath.Join(resourcesDir, "chrome_200_percent.pak"),
		filepath.Join(resourcesDir, "resources.pak"),
		filepath.Join(resourcesDir, "icudtl.dat"),
	}
...
func cefCommandLineArgs(base []string, executable string, layout linuxRuntimeLayout) []string {
	...
	args = ensureSwitch(args, "--framework-dir-path="+layout.frameworkRoot)
